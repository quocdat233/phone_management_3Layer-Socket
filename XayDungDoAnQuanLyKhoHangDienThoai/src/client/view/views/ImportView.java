...
public class ImportView extends BaseView {
    ...
    public ImportView() {
        ...
        table.setBackground(Color.WHITE);
        updateTable(phieuNhapDAO.layDanhSachPhieuNhap());
    }
    ...
    private void initLeftPanel() {
        ...
        JComboBox<String> supplierComboBox = new JComboBox<>(new String[]{"Tất cả", "Nhà cung cấp A", "Nhà cung cấp B"});
    ...
    public void updateTable(List<?> list) {
        ...
        if (!(list.get(0) instanceof phieuNhap)) return;

        List<phieuNhap> phieuNhapList = (List<phieuNhap>) list;

        DefaultTableModel model = (DefaultTableModel) getTable().getModel();
        model.setRowCount(0);

        for (phieuNhap sp : phieuNhapList) {
            model.addRow(new Object[]{
                    null, // STT sẽ được render tự động
                    sp.getMaphieu(),
                    sp.getTennhacungcap(),
                    sp.getTennhanvien(),
                    sp.getThoigiantao(),
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new ImportView().setVisible(true));
    }
}
