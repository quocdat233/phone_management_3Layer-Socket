...
public class ThongKeDAO {
    ...
    public static HashMap<Integer, ArrayList<ThongKeTonKho>> getThongKeTonKho(String text, Date timeStart, Date timeEnd) {
        HashMap<Integer, ArrayList<ThongKeTonKho>> result = new HashMap<>();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ThongKeTonKho p = new ThongKeTonKho(masp, maphienbansp, tensp, ram, rom, mausac, soluongdauky, soluongnhap, soluongxuat, soluongcuoiky);
    ...
    public ArrayList<ThongKeDoanhThu> getDoanhThuTheoTungNam(int year_start, int year_end) {
        ArrayList<ThongKeDoanhThu> result = new ArrayList<>();
        try {
            Connection con = Database.getConnected();
            ...
            while (rs.next()) {
                ...
                ThongKeDoanhThu x = new ThongKeDoanhThu(thoigian, chiphi, doanhthu, doanhthu - chiphi);
                result.add(x);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static ArrayList<ThongKeKhachHang> getThongKeKhachHang(String text, Date timeStart, Date timeEnd) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ThongKeKhachHang x = new ThongKeKhachHang(makh, tenkh, soluong, tongtien);
                result.add(x);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static ArrayList<ThongKeNhaCungCap> getThongKeNCC(String text, Date timeStart, Date timeEnd) {
        ArrayList<ThongKeNhaCungCap> result = new ArrayList<>();
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(timeEnd.getTime());
        // Đặt giá trị cho giờ, phút, giây và mili giây của Calendar
        calendar.set(Calendar.HOUR_OF_DAY, 23);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ThongKeNhaCungCap x = new ThongKeNhaCungCap(mancc, tenncc, soluong, tongtien);
                result.add(x);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public ArrayList<ThongKeTungNgayTrongThang> getThongKeTungNgayTrongThang(int thang, int nam) {
        ArrayList<ThongKeTungNgayTrongThang> result = new ArrayList<>();
        try {
            ...
            while (rs.next()) {
                ...
                int doanhthu = rs.getInt("doanhthu");
                int loinhuan = doanhthu - chiphi;
                ThongKeTungNgayTrongThang tn = new ThongKeTungNgayTrongThang(ngay, chiphi, doanhthu, loinhuan);
                result.add(tn);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public ArrayList<ThongKeTungNgayTrongThang> getThongKeTuNgayDenNgay(String star, String end) {
        ArrayList<ThongKeTungNgayTrongThang> result = new ArrayList<>();
        try {
            Connection con = Database.getConnected();
